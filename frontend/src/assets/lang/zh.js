...
export default {
  ...
  kb: {
    qa: {
      ...
      test: "测试问答",
      testPH: "请输入一些文本",
      empty: "暂无问答对，点击\"新增问答对\"创建一条。",
      delConfirm: "确认要删除该问答吗?",
      table: ["标准问", "相似问数量", "答案", "操作"],
      form: {
        question: "标准问",
        questionPH: "用户可能会问的问题",
        similar: "相似问",
        similarPH: "该问题的其他问法",
        addSimilar: "新增相似问",
        similarTip: "相似问越多，匹配越准确",
        answer: "答案",
        answerPH: "回复给用户的答案",
      },
      rules: {
  ...
  settings: {
    ...
    ipNote2:
      "如果配置的IP地址错误导致应用启动失败, 请在启动时, 加上 -rs 来重置配置参数",
    prompt2: "应用端口",
    prompt2_2: "当指定端口被占用, 随机选择一个可用的端口",
    note: "修改了IP地址, 端口或会话时长，需要重启应用才能生效",
    invalidIp: "设置的IP地址不正确",
    commonSettings: "通用设置",
    listeningIp: "监听 IP 地址 (v4 或 v6)",
    hfDownload: "HuggingFace 模型下载设置",
    hfDownloadTip: "从 HuggingFace.co 下载模型文件时使用这些设置。部分模型（如 Gemma、Llama）需要访问令牌才能下载。",
    hfAccessToken: "访问令牌",
    hfAccessTokenHelp: "用于下载 HuggingFace.co 上需要授权的模型。可在 https://huggingface.co/settings/tokens 创建。",
  },
  botSettings: {
    prompt3: "会话时长",
    prompt4: "分钟",
    ...
    cancelLower: "取消",
    chooseModel: "请选择模型",
    providerOpenAiCompatible: "在线模型",
    providerHuggingFace: "本地模型",
    ...
    fetchModelListOk: "已获取 {count} 个模型",
    ...
    smtp: {
      ...
      password: "密码",
      test: "测试 SMTP 设置",
      testPassed: "SMTP 测试通过",
      testFailed: "SMTP 测试失败",
      emailRegex: "邮箱验证正则表达式",
      emailRegexHelp: "您可以自定义邮箱验证正则表达式，留空则系统自动使用通用验证规则。",
    },
  },
  var: {
    types: ["字符串", "数字"],
  ...
  eApi: {
    ...
    detail: {
      ...
      addFormData: "添加表单数据",
      noHeaders: "暂无请求头，点击“添加请求头”新建一个。",
      noQueryParams: "暂无查询参数，点击“添加查询参数”新建一个。",
      noFormData: "暂无表单数据，点击“添加表单数据”新建一个。",
      varSource: "变量",
      userAgent: "User-Agent",
      test: "测试（开发中）",
      addHeaderTitle: "添加请求头参数",
      addQueryParamTitle: "添加查询参数",
      addFormTitle: "添加 POST 参数",
      editHeaderTitle: "编辑请求头参数",
      editQueryParamTitle: "编辑查询参数",
      editFormTitle: "编辑 POST 参数",
      pName: "名称",
      pValue: "值",
      constValue: "常量值",
      fromVar: "从变量取值",
  ...
  home: {
    ...
    introTitle: "这是什么软件?",
    intro1:
      "它类似 Google 的 Dialogflow, 但是多了一个流程画布编辑器，可以更好的设计流程. 它也像 Typebot, 但是多了一个完整的应答后端.",
    intro2:
      "它拥有一个可视化的流程编辑器, 编辑完成后，可以测试流程, 并最终发布流程.",
    intro3:
      "目前，它可以返回话术给用户，并采集用户输入，还可以通过条件判断，执行不同的节点.",
    intro4:
      "它很轻量。整个软件，包含了前端和后端，只有不到 6M 大小，非常易于分发和部署.",
    intro5:
      "你可以修改软件的监听端口，这样就可以在同一台服务器上，同时运行多个实例，解决不同的用户场景.",
    intro6:
      "它下载后，就可以直接使用，不用安装任何依赖。而且数据是存放在本地，可以保护数据营隐私.",
    midTitle: "我们的优势",
    adv1Title: "易用",
    ...
    demo3: "一句话通知",
    demoUnvailableTitle:
      "演示在Github上不可用Demos are not available on Github",
    demoUnvailableContent:
      '由于目前没有服务器来托管后台.<br />但是可以<a href="https://github.com/dialogflowai/dialogflow/releases">下载该软件</a>, 它包含了3个演示对话流程',
    adv2Title: "小巧、快速",
  ...
  guide: {
    ...
    intentsDesc:
      '我们内建了 "肯定" 和 "否定" 意图. 如果不够, 您可以自行添加更多意图。',
